...
typedef struct _TIMED_TEXT_INIT_STRUCT
{
	...
	bool useDroppedShadow;		// display a dropped shadow?  true = yes
...
public:
	...
	void Start()   { SETFLAG( m_flags, CTIMEDTEXTFLAG_RUNNING );	m_prevUpdateTime = g_pLTClient->GetTime(); ASSERT( m_text );  m_fLineYPos = m_text->GetPolys()[ 0 ].verts[ 0 ].pos.y; }

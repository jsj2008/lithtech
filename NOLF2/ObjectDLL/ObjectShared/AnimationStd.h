...
/*
	...
	template <class t_ClsKey, class t_ClsValue, uint32 t_nSize>
	LTBOOL CMap<t_ClsKey, t_ClsValue, t_nSize>::Find(const t_ClsKey& clsKey, t_ClsValue** ppclsValue ) const
	{
		uint32 iEntry = GetFnHash().Hash(clsKey)%t_nSize;
		CMapEntry* pEntry = m_apEntries[iEntry];
		while ( pEntry )
...
*/
	...
	template <uint32 t_nSize, LTBOOL t_bCaseInsensitive = LTTRUE>
		class CMapStrStr : public CMap<std::string, std::string, t_nSize>
	{
		private :

			// Functors

			struct FnHashCStr : public CMap<std::string, std::string, t_nSize>::FnHash
			{
				uint32 Hash(std::string const& sString) const
				{
					return t_bCaseInsensitive ? toupper(sString[0]) : sString[0];
				}
			};

			struct FnCompareCStr : public CMap<std::string, std::string, t_nSize>::FnCompare
			{
				LTBOOL Compare(std::string const& sString1, std::string const& sString2) const
				{
					return !t_bCaseInsensitive ? !strcmp(sString1.c_str(), sString2.c_str()) : !stricmp(sString1.c_str(), sString2.c_str());
				}
			};
	...
	template <class t_Cls, uint32 t_nSize, LTBOOL t_bCaseInsensitive = LTTRUE>
	class CMapStrCls : public CMap<std::string, t_Cls, t_nSize>
	{
		private :

			// Functors

			struct FnHashCStr : public CMap<std::string, t_Cls, t_nSize>::FnHash
			{
                uint32 Hash(std::string const& sString) const
				{
                    LPCTSTR temp = sString.c_str();
                    return t_bCaseInsensitive ? toupper(temp[0]) : temp[0];
				}
			};

			struct FnCompareCStr : public CMap<std::string, t_Cls, t_nSize>::FnCompare
			{
				LTBOOL Compare(std::string const& sString1, std::string const& sString2) const
				{
					return !t_bCaseInsensitive ? !strcmp(sString1.c_str(), sString2.c_str()) : !stricmp(sString1.c_str(), sString2.c_str());

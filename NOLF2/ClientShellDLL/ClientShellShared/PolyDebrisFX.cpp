...
LTBOOL CPolygonDebrisFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	debris.rRot				= LTRotation(m_cs.vNormal, LTVector(0.0f, 1.0f, 0.0f));
	...
	debris.vMinWorldVel		= m_cs.PolyDebrisFX.vMinWorldVel;
	debris.vMaxWorldVel		= m_cs.PolyDebrisFX.vMaxWorldVel;
...
void CPolygonDebrisFX::CreateDebris(int i, const LTVector &vPos)
{
    ...
    if (hCamera)
	{
		...
		vCamPos -= vPos;
		LTFLOAT fScaleVal = vCamPos.Length() / g_cvarPolyDebrisScaleDist.GetFloat();

		fScaleVal = LTCLAMP(fScaleVal, g_cvarPolyDebrisMinDistScale.GetFloat(), g_cvarPolyDebrisMaxDistScale.GetFloat());
...
void CPolygonDebrisFX::SetDebrisPos(int i, const LTVector &vPos)
{
	if (i < 0 || i >= m_ds.nNumDebris) return;
	if (i >= m_nNumPolies) return;
	if (m_Polies[i] == LTNULL) return;
...
void CPolygonDebrisFX::SetDebrisRot(int i, const LTRotation &rRot)
{
	if (i < 0 || i >= m_ds.nNumDebris) return;
	if (i >= m_nNumPolies) return;
	if (m_Polies[i] == LTNULL) return;

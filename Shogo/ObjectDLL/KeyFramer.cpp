...
{
	CServerDE* pServerDE = GetServerDE();
	if (!pServerDE || m_bActive || !m_pKeys || 
		!m_pObjectList || (m_pObjectList->m_nInList < 1)) return;
...
{
	CServerDE* pServerDE = GetServerDE();
	if (!pServerDE || !m_hstrObjectName || m_pObjectList) return DFALSE;
	...
	if (m_pObjectList)
	{
		...
		else
		{
			pServerDE->RelinquishList(m_pObjectList);
			m_pObjectList = DNULL;
			return DFALSE;  // No objects, no reason to go on...
		}	
	}
	...
	while (1)
	{
		...
		SAFE_STRCPY(strKey, pServerDE->GetStringData (m_hstrBaseKeyName));
		strcat (strKey, strNum);
			...
			if (m_pObjectList && hLink)
			{
				...
				while (pLink)
				{
					if (pLink->m_hObject == hLink)
					{
						...
						{
							m_pObjectList->m_pFirstLink = DNULL;
						}
						else if (pPrevious)
						{
...
DBOOL KeyFramer::ReadProp(ObjectCreateStruct *)
{
	CServerDE* pServerDE = GetServerDE();
...
DBOOL KeyFramer::Update(DVector* pMovement)
{
	...
	{
		if (m_fCurTime >= m_pPosition1->keyData.m_fRealTime && m_fCurTime <= m_pPosition2->keyData.m_fRealTime)
		{
			float t = (m_fCurTime - m_pPosition1->keyData.m_fRealTime) / (m_pPosition2->keyData.m_fRealTime - m_pPosition1->keyData.m_fRealTime);
			...
			long i = 0;
			ObjectLink* pLink = m_pObjectList ? m_pObjectList->m_pFirstLink : DNULL;
			while (pLink)
			{
...
void KeyFramer::Save(HMESSAGEWRITE hWrite, DDWORD dwSaveFlags)
{
	...
	DBYTE nNumInList = m_pObjectList ? m_pObjectList->m_nInList : 0;
	pServerDE->WriteToMessageByte(hWrite, nNumInList);
	...
	if (m_pObjectList && nNumInList)
	{
		...
		while (pLink)
		{
			pServerDE->WriteToLoadSaveMessageObject(hWrite, pLink->m_hObject);
			pLink = pLink->m_pNext;
		}
	}
}
...
void KeyFramer::Load(HMESSAGEREAD hRead, DDWORD dwLoadFlags)
{
	...
	DBYTE nNumInList = pServerDE->ReadFromMessageByte(hRead);
	...
	if (nNumInList > 0)
	{
		...
		m_pObjectList = pServerDE->CreateObjectList();

		if (m_pObjectList)
		{
			HOBJECT* hObjectArray = new HOBJECT[nNumInList];
			...
			for (int i=nNumInList-1; i >= 0; i--)
			{
				if (hObjectArray[i])
				{
					pServerDE->AddObjectToList(m_pObjectList, hObjectArray[i]);
					pServerDE->CreateInterObjectLink(m_hObject, hObjectArray[i]);
				}
			}

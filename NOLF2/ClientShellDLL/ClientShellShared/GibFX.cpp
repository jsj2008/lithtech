...
void CGibFX::UpdateGib(int nIndex, LTBOOL bBounced)
{
	...
	if (m_Emitters[nIndex].m_dwPhysicsFlags & MO_RESTING)
	{
		...
		if (m_hGib[nIndex])
		{
			if (m_bRotate)
			{
				...
				rRot.Rotate(rRot.Forward(), m_fRoll);
				g_pLTClient->SetObjectRotation(m_hGib[nIndex], &rRot);
			}
	...
	{
		g_pLTClient->SetObjectPos(m_hGib[nIndex], &(m_Emitters[nIndex].m_vPos));
...
HLOCALOBJ CGibFX::CreateGib(GibType eType)
{
    ...
    char* pFilename = LTNULL;//GetGibModel(m_eModel, eType);
    char* pSkin     = LTNULL;//GetGibSkin(m_eModel, eType);
...
void CGibFX::HandleBounce(int nIndex)
{
	...
	if (m_bBloodSplats)
	{
		...
		sc.rRot = LTRotation(m_info.m_Plane.m_Normal, LTVector(0.0f, 1.0f, 0.0f));

		sc.vPos = m_info.m_Point + (LTVector)m_info.m_Plane.m_Normal * 2.0f; // Off the wall/floor a bit
		sc.vVel.Init();
...
LTBOOL CGibFX::OkToRemoveGib(HLOCALOBJ hGib)
{
	...
	g_pLTClient->GetObjectPos(hGib, &vPos);

...
LTBOOL CPVFXMgr::Init(HOBJECT hModelObj, WEAPON const *pWeapon)
{
...
void CPVFXMgr::DisableAllFX()
{
    ...
    for (i=0; i < PVFX_MAX_SCALE_FX; i++)
	{
		...
		if (hFX)
		{
			g_pCommonLT->SetObjectFlags(hFX, OFT_Flags, 0, FLAG_VISIBLE);
		}
...
void CPVFXMgr::Update()
{
	...
	// get a pointer to the current client weapon
	...
	if (!g_pPlayerMgr->IsFirstPerson() ||
		...
		( pClientWeapon && !pClientWeaponMgr->WeaponsEnabled() ) )
	{
        bForceHide = LTTRUE;
	}
    ...
    for (i=0; i < PVFX_MAX_SCALE_FX; i++)
	{
		if (m_ScaleFX[i].bUsed && m_ScaleFX[i].hSocket != INVALID_MODEL_SOCKET)
		{
			...
			g_pLTClient->SetObjectPos(hFX, &vPos);
			...
			g_pCommonLT->SetObjectFlags(hFX, OFT_Flags, (m_ScaleFX[i].bOn && !bForceHide) ? FLAG_VISIBLE : 0, FLAG_VISIBLE);
		}
	}
	...
	for (i=0; i < PVFX_MAX_DLIGHT_FX; i++)
	{
		if (m_DLightFX[i].bUsed && m_DLightFX[i].hSocket != INVALID_MODEL_SOCKET)
		{
			...
			LTVector vOffset;
			g_pLTClient->GetObjectPos(g_pPlayerMgr->GetCamera(), &vOffset);
			...
			g_pCommonLT->SetObjectFlags(hFX, OFT_Flags, (m_DLightFX[i].bOn && !bForceHide) ? FLAG_VISIBLE : 0, FLAG_VISIBLE);
		}
	}
}

		...
		if ( m_state == CTIMEDTEXT_TIMING )
		{
			if ( TESTFLAG( m_flags, CTIMEDTEXTFLAG_SCROLLING ) )
			{
				...
				else
				{
					...
					m_fLineYPos = m_text->GetPolys()[ textPos ].verts[ 0 ].pos.y;
			...
			else if ( ( m_text->GetPolys()[ textPos ].verts[ 0 ].pos.y - m_fLineYPos ) > 0.5f ) 
			{
...
void CTimedText::Render()
{
	...
	if ( TESTFLAG( m_flags, CTIMEDTEXTFLAG_USE_DROPPED_SHADOW ) )
	{
		...
		m_text->SetColor( SETRGBA( 0, 0, 0, alpha ) );

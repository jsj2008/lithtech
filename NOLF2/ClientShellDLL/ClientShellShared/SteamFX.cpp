...
LTBOOL CSteamFX::Init(HLOCALOBJ hServObj, ILTMessage_Read *pMsg)
{
...
LTBOOL CSteamFX::Update()
{
	...
	if (m_hServerObject)
	{
		...
		if (m_hSound)
		{
            ...
            ((ILTClientSoundMgr*)g_pLTClient->SoundMgr())->SetSoundPosition(m_hSound, &vPos);
		}
...
void CSteamFX::StartSound()
{
	if (!m_hSound && m_cs.hstrSoundName)
	{
        ...
        const char* pSnd = g_pLTClient->GetStringData(m_cs.hstrSoundName);
...
void CSteamFX::StopSound()
{
	if (m_hSound)
	{
        g_pLTClient->SoundMgr()->KillSoundLoop(m_hSound);
        m_hSound = LTNULL;
	}
}

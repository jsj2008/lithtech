...
CNinfoGame* CNinfoMgr::AddGame(const char* sInfo, NetSession* pNetSession, DWORD dwNumDPlayPlayers)
{
	...
	if (!pGame->Init(sName, sHost, sLevel, nType, pNetSession, dwNumDPlayPlayers, pNetSession->m_HostIP, pNetSession->m_HostPort))
	{
		delete pGame;
		return(NULL);
	}
...
BOOL CNinfoGame::Init(char* sName, char* sHost, char* sLevel, int nType, NetSession* pNetSession, DWORD dwNumDPlayPlayers, char *pHostIP, DWORD hostPort)
{
	...
	switch (m_nType)
	{
		case NGT_DEATHMATCH:	strcpy(m_sType, "Deathmatch"); break;
		case NGT_CAPTUREFLAG:	strcpy(m_sType, "Capture the Flag"); break;
		case NGT_COOPERATIVE:	strcpy(m_sType,	"Cooperative"); break;

		default: strcpy(m_sType, "Unknown");
	}


	// Calculate a CRC value...

	m_dwCRC = m_pNetSession->m_guidInst.guid.a;

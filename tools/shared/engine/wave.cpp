...
static const uint32 s_tagWAVH = mmioFOURCC( 'w', 'a', 'v', 'h' );
...
static LTBOOL ReadChunk( ILTStream &inStream, uint32 dwType, uint32 dwLength, uint32 dwForm, CWaveHeader &waveHeader )
{
	...
	switch( dwType )
	{
		case s_tagFMT:
		{
			if( dwForm == s_tagWAVE )
			{
				uint32 nSizeToRead = sizeof(LTWAVEFORMATEX);
		...
		case s_tagWAVH:
		{
			uint32 nSizeToRead = sizeof( DMUS_IO_WAVE_HEADER );
			if ( dwLength < nSizeToRead )
				nSizeToRead = dwLength;
			inStream.Read( &waveHeader.m_dmus.m_dmusWavHeader, nSizeToRead );
			if ( waveHeader.m_dmus.m_dmusWavHeader.dwFlags & DMUS_WAVEF_STREAMING )
				waveHeader.m_dmus.m_bStream = TRUE;
			else
				waveHeader.m_dmus.m_bStream = FALSE;
			break;
		}
...
static LTBOOL WriteChunk( SSBufStream &inStream, ILTStream &outStream, uint32 dwType, uint32 dwLength, uint32 dwForm, CWaveHeader &waveHeader )
{
	...
	switch( dwType )
	{
		...
		case s_tagLIST:
		{
			...
			if ( dwType == s_tagRIFF )
			{
				if ( waveHeader.m_dmus.m_bStream )
				{
					outStream.Write(( void * )&s_tagWAVH, sizeof( uint32 ));
					dwTemp = sizeof( DMUS_IO_WAVE_HEADER );
					outStream.Write(( void * )&dwTemp, sizeof( uint32 ));
					outStream.Write(( void * )&waveHeader.m_dmus.m_dmusWavHeader, dwTemp );
				}
			}

...
public:
    ...
    void    SetObjPos(FXType *pType, FXObj *pObj, const LTVector &pos)
	{
		g_pLTClient->SetObjectPos(pObj->m_hModel, const_cast<LTVector*>(&pos));
	}
    ...
    void    SetObjColor(FXObj *pObj, const LTVector &color)
	{
		float r, g, b, a;
	...
	{
		g_pCommonLT->SetObjectFlags(pObj->m_hModel, OFT_Flags, FLAG_VISIBLE, FLAG_VISIBLE);
	}

	void	HideObj(FXObj *pObj)
	{
		g_pCommonLT->SetObjectFlags(pObj->m_hModel, OFT_Flags, 0, FLAG_VISIBLE);
	}
...
public:
    ...
    LTVector GetObjPos(FXType *pType, FXObj *pObj)
	{
		return pObj->m_pParticle->m_Pos;
	}

    void    SetObjPos(FXType *pType, FXObj *pObj, const LTVector &pos)
	{
		pObj->m_pParticle->m_Pos = pos;
	}
...
FXType::~FXType()
{
	...
	if(m_pObjects)
	{
		if(m_ObjType == FXObj_Model)
		{
			...
			{
                g_pLTClient->RemoveObject(m_pObjects[i].m_hModel);
			}
		}
...
LTBOOL SprinklesFX::Init(HLOCALOBJ hServObj, ILTMessage_Read *pMsg)
{
...
LTBOOL SprinklesFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	for(i=0; i < m_nFXTypes; i++)
	{
        ...
        const char *pTemp = g_pLTClient->GetStringData(pConstruct->m_hFilename);
	...
	for(i=0; i < m_nFXTypes; i++)
	{
		...
		for(pCur=pType->m_ActiveList.m_pNext; pCur != &pType->m_ActiveList; pCur=pNext)
		{
			...
			if(pType->m_ObjType == FXObj_Model)
			{
			    ...
			    LTRotation rot(VEC_EXPAND(pFXObj->m_Angles));
			...
			// Set its alpha.
			alphaVal = 1.0f - ((curPos - playerPos).Length() / pType->m_SpawnRadius);

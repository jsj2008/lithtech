...
uint32		CCredit::s_dwScreenWidth    = 0;
uint32		CCredit::s_dwScreenHeight   = 0;
HLTFONT		CCredit::s_hFont            = NULL;
HSURFACE	CCredit::s_hScreen          = NULL;
ILTClient*	CCredit::s_pClientDE        = NULL;
LTFLOAT		CCredit::s_fDefaultHoldTime = 1.6f;
LTFLOAT		CCredit::s_fDefaultOutTime  = 0.5;
LTBOOL		CCredit::s_bClearScreen     = LTTRUE;
...
LTBOOL CCredit::Init(ILTClient* pClientDE, char* sBuf)
{
	// Sanity checks...
...
LTBOOL CCredit::AddString(char* sString)
{
	...
	if (_mbsnbcmp((const unsigned char*)sString, (const unsigned char*)">TIME:", 6) == 0)
	{
		...
		{
			m_fHoldTime = (LTFLOAT)atof(&sString[6]);
			return(LTTRUE);
		}
	}
...
LTBOOL CCredit::SetStaticInfo(ILTClient* pClientDE, int nMode, LTBOOL bClearScreen)
{
	// Sanity checks...
	...
	int xFont = TextHelperGetIntValFromStringID(pClientDE, IDS_CREDITSTEXTWIDTH, 8);
	int yFont = TextHelperGetIntValFromStringID(pClientDE, IDS_CREDITSTEXTHEIGHT, 14);

	if (s_dwScreenWidth > 360)
	{
		xFont = TextHelperGetIntValFromStringID(pClientDE, IDS_CREDITSTEXTWIDTH360, 10);
		yFont = TextHelperGetIntValFromStringID(pClientDE, IDS_CREDITSTEXTHEIGHT360, 16);
	}

	if (s_dwScreenWidth > 520)
	{
		xFont = TextHelperGetIntValFromStringID(pClientDE, IDS_CREDITSTEXTWIDTH520, 16);
		yFont = TextHelperGetIntValFromStringID(pClientDE, IDS_CREDITSTEXTHEIGHT520, 22);
	}
	...
	s_hFont = pClientDE->CreateFont(pClientDE->GetStringData(hstrFont), xFont, yFont, LTFALSE, LTFALSE, LTFALSE);
	pClientDE->FreeString (hstrFont);
	if (!s_hFont)
	{
		s_hFont = pClientDE->CreateFont("Arial", xFont, yFont, LTFALSE, LTFALSE, LTFALSE);
		return(LTFALSE);
	}
	...
	if (nMode == CM_CREDITS)
	{
		s_fDefaultHoldTime = 2.0f;
		s_fDefaultOutTime = 0.5f;
	}

	if (nMode == CM_INTRO)
	{
		s_fDefaultHoldTime = 3.0f;
...
void CCredit::Draw(int x, int y, HLTCOLOR hForeColor)
{
	// Sanity checks...
	...
	{
		HLTCOLOR hBackColor = m_pClientDE->SetupColor2(0, 0, 0, LTFALSE);
		DrawString(GetString(i), hForeColor, hBackColor, x, yCur);
		yCur += (s_nFontHeight + s_nFontSpacing);
	}
}

void CCredit::DrawString(HSTRING hString, HLTCOLOR hForeColor, HLTCOLOR hBackColor, int x, int y)
{
	// Sanity checks...
...
LTBOOL CCredit::UpdateFadeIn()
{
	...
	HLTCOLOR hColor = m_pClientDE->SetupColor1(m_fColor, m_fColor, m_fColor, LTFALSE);
...
LTBOOL CCredit::UpdateFadeOut()
{
	...
	if (!s_bClearScreen && m_fColor < 0.1f) m_fColor = 0.1f;
...
LTBOOL CCredits::Init(ILTClient* pClientDE, CRiotClientShell* pClientShell, int nMode, LTBOOL bClearScreen)
{
	// Sanity checks...
...
void CCredits::DrawCenteredStringToScreen(HLTFONT hFont, HSTRING hString, HLTCOLOR hForeColor, HLTCOLOR hBackColor, int xDraw, int yDraw)
{
	// Sanity checks...
...
void CCredits::AddCredits()
{
	...
	char* sBuf = CreditsWin_GetTextBuffer(sName);
	if (!sBuf) return;

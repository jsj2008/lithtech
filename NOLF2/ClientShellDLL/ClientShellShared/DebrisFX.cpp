...
LTBOOL CDebrisFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	if (pDebris)
	{
		...
		strcpy(m_ds.szWorldSpaceFX, pDebris->szWorldSpaceFX);
		strcpy(m_ds.szImpactSpaceFX, pDebris->szImpactSpaceFX);
	}
...
LTBOOL CDebrisFX::CreateObject(ILTClient *pClientDE)
{
    ...
    LTVector vVelMin, vVelMax, vU, vR, vF;

	vU = m_ds.rRot.Up();
	vR = m_ds.rRot.Right();
	vF = m_ds.rRot.Forward();

	vVelMin = m_ds.vMinWorldVel;
	vVelMax = m_ds.vMaxWorldVel;
	...
	//create the client effects
	if ( m_ds.szWorldSpaceFX[ 0 ] != '\0' )
	{
		// prepare the create struct
		CLIENTFX_CREATESTRUCT fxInit(m_ds.szWorldSpaceFX, 0, m_ds.vPos);

		// create the client fx
		g_pClientFXMgr->CreateClientFX(NULL, fxInit, true);
	}
	if ( m_ds.szImpactSpaceFX[ 0 ] != '\0' )
	{
		// prepare the create struct
		CLIENTFX_CREATESTRUCT fxInit(m_ds.szImpactSpaceFX, 0, m_ds.vPos, m_ds.rRot);
			...
			else if (IsValidDebris(i))
			{
				...
				if (m_ds.bRotate)
				{
					...
					if (pTracker->m_fPitchVel != 0 || pTracker->m_fYawVel != 0 || pTracker->m_fRollVel != 0)
					{
                        ...
                        LTRotation rRot(pTracker->m_fPitch, pTracker->m_fYaw, pTracker->m_fRoll);
...
LTBOOL CDebrisFX::OkToRemoveDebris(int i)
{
    ...
    if (vDir.Dot(rRot.Forward()) <= 0.0f) return LTTRUE;
...
void CDebrisFX::RotateDebrisToRest(int i)
{
    ...
    LTRotation rRot;
	rRot.Rotate(LTVector(0.0f, 1.0f, 0.0f), pTracker->m_fYaw);
	g_pLTClient->SetObjectRotation(pTracker->m_hDebris, &rRot);
}
...
void CDebrisFX::SetDebrisPos(int i, const LTVector &vPos)
{
	if (i < 0 || i >= m_ds.nNumDebris) return;
	...
	g_pLTClient->SetObjectPos(pTracker->m_hDebris, const_cast<LTVector*>(&vPos));
}
...
LTBOOL CDebrisFX::GetDebrisPos(int i, LTVector & vPos)
{
	...
	g_pLTClient->GetObjectPos(pTracker->m_hDebris, &vPos);
...
void CDebrisFX::SetDebrisRot(int i, const LTRotation &rRot)
{
	if (i < 0 || i >= m_ds.nNumDebris) return;
	...
	g_pLTClient->SetObjectRotation(pTracker->m_hDebris, const_cast<LTRotation*>(&rRot));
}
